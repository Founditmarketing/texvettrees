...
export function Gallery() {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      else if (e.key === 'End') { setDirection(1); setOpenIndex(visible.length - 1); }
    };
  ...
  }, [openIndex, step, visible.length, close]);
    ...
    <section id="gallery" className="relative py-24 bg-[#0a0a0a] border-t border-white/5">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="flex flex-col items-center text-center mb-10">
          ...
          >
            Real, finished job sites across Central &amp; North Texas. Filter by service to explore.
          </motion.p>
        </div>

        {/* Filter chips */}
        <div role="group" aria-label="Filter projects by service" className="flex flex-wrap justify-center gap-2 sm:gap-3 mb-10">
          {CATEGORIES.map((cat) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2 text-[11px] sm:text-xs font-bold uppercase tracking-widest rounded-full border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4c5230] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0a0a0a] ${
                  isActive
                    ? 'bg-[#4c5230] border-[#4c5230] text-white'
                    : 'border-white/15 text-white/60 hover:text-white hover:border-white/40 bg-white/[0.02]'
                }`}
              >
                {cat}
                <span className={`ml-1.5 ${isActive ? 'text-white/70' : 'text-white/40'}`}>{count}</span>
              </button>
            );
          })}
        </div>
        ...
        <p className="sr-only" aria-live="polite">
          {`Showing ${visible.length} ${active === 'All' ? 'projects' : `${active} projects`}`}
        </p>
      </div>
        ...
        <AnimatePresence>
          {current && openIndex !== null && (
            <motion.div
              ...
              role="dialog"
              aria-modal="true"
              aria-label={`Image ${openIndex + 1} of ${visible.length}`}
            >
